...
const recentLogs = [];
let totalLogged = 0;
let logDir = process.env.LOG_DIR || '/srv/threadripper-backups/newapi/logs';
let serviceName = 'default';

function getLogPath(date) {
  const dir = join(logDir, serviceName);
  mkdirSync(dir, { recursive: true });
  return join(dir, `${date || new Date().toISOString().slice(0, 10)}.jsonl`);
}
  ...
  return (req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      if (isChat && messages && res.statusCode === 200) {
        ...
        if (stream) {
          assistantContent = parseSSEChunks(chunks);
        } else {
          try {
